...
const Component = () => {
  ...
  const filteredVacancy = _.filter(getAllVacancies, filters);

  const getFields = (field) => {
    if(getAllVacancies.length === 0) return; 
    return getAllVacancies.reduce( (result, object) => {
      if(result.find( name => name.value === object[field].name)) return;
      (Array.isArray(result) || (result = [])).push({value: object[field].name, label: object[field].name})
      return result; 
    })
  };

  console.log( window.p = filteredVacancy, window._ = _, window.op = getFields );
  console.log( _.filter(getAllVacancies, filters));
      ...
      <Row>
        <Col lg="12" className="mb-3">
          <Row>
            ...
            <Col lg="3">
              <Input onChange={ 
                event => setVacanciesFiltered( event.target.value ? fuse.search(event.target.value) : getAllVacancies ) 
              }
              />
            </Col>
            <Col lg="2">
